...
const AppWithoutTranslation = ({ root_store }) => {
    ...
    React.useEffect(() => {
        const handleAuthSync = (event) => {
            ...
            if (event.data?.type === 'DTRADER_AUTH') {
                const { token, accountsList, clientAccounts, active_loginid, app_id } = event.data;
                ...
                // If account changed, trigger a re-authorization in DTrader
                if (root_store.client.loginid !== active_loginid) {
        ...
        window.parent.postMessage({ type: 'DTRADER_READY' }, '*');
